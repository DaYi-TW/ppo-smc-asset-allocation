"""Integration test fixtures for 005-inference-service.

提供 ServiceConfig + InferenceState + httpx AsyncClient 的共用 fixtures。
"""
...
def fake_payload_dict() -> dict[str, Any]:
    """A canonical PredictionPayload dict for stubbing handler."""
    return {
...
@pytest.fixture
def fake_state() -> Any:
    """A pre-built InferenceState with mocked policy + env_factory.

    回傳前不跑 init_state（避免載入 ~200 MB policy.zip + warmup env）。
    """
