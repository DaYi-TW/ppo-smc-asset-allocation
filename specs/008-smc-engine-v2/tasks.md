...
- [ ] T002 [P] 在 `tests/conftest.py`（若不存在則新建）加入共用 fixture `make_random_ohlcv(seed: int, n: int)` 供等價性測試使用 — 對應 contracts/incremental.contract.md 的 randomized 等價測試方案
...
**⚠️ CRITICAL**：T003 ~ T006 完成前任何 user story phase 的 implementation task 都不能開始。
...
- [ ] T008 修改 `src/smc_features/types.py`：擴充 `BatchResult` 加 `breaks: list[StructureBreak]` 欄位 — 對應 spec FR-007
- [ ] T009 修改 `src/smc_features/types.py`：擴充 `IncrementalState` 加 `used_swing_high_bar_indices: set[int]`、`used_swing_low_bar_indices: set[int]` 兩欄位（注意此 dataclass 為 mutable，不 frozen）— 對應 spec FR-003、FR-014
- [ ] T010 修改 `src/smc_features/__init__.py`：對外 export `StructureBreak`、`BreakKind`、`TrendState`
- [ ] T011 跑 T003 + T004 + 既有所有測試，確認 dataclass 擴充無破壞既有 contract（`pytest tests/unit/test_types.py tests/contract/test_event_schema.py tests/unit/test_swing.py -v`）
...
- [ ] T039 [P] [US4] 在 `tests/unit/test_batch.py` 新增 `test_breaks_signal_array_consistency`：對 NVDA 1000-bar 切片驗證 contract Invariant B-1、B-2 — 對應 contracts/batch_compute Invariant B-1/B-2
- [ ] T040 [P] [US4] 在 `tests/unit/test_batch.py` 新增 `test_obs_aligned_with_breaks`：每個 OB 的 `source_break_index` 合法、`source_break_kind` 一致、direction 對齊 — 對應 contract Invariant B-3
- [ ] T041 [P] [US4] 在 `tests/unit/test_batch.py` 新增 `test_dedup_anchor_swing_unique`：每個 (anchor_swing_bar_index, direction) 組合在 breaks 中至多一次 — 對應 contract Invariant B-4
- [ ] T042 [P] [US4] 在 `tests/unit/test_batch.py` 新增 `test_fvg_atr_filter_in_batch_result`：BatchResult.fvgs 中每個 FVG 滿足 ATR 過濾條件 — 對應 contract Invariant B-5
- [ ] T043 [P] [US4] 在 `tests/contract/test_smc_observation.py`（新建）寫 `test_observation_5_channel_shape_unchanged`：用 BatchResult 組 5-channel float32 array → 驗證 shape `(n, 5)`、dtype float32 — 對應 spec FR-013、Constitution Gate III-2、contract Invariant B-6
- [ ] T044 [P] [US4] 在 `tests/unit/test_batch.py` 新增 `test_batch_compute_deterministic`：同輸入跑兩次，產出 byte-identical（用 `np.array_equal` + dataclass `==`）— 對應 contract Invariant B-7、SC-006
- [ ] T045 [P] [US4] 在 `tests/unit/test_incremental.py` 改寫 `test_batch_incremental_equivalence_random_seeds`：parametrize seeds=[0,1,2,42,1337]、n=1000 隨機 OHLCV → 比對 breaks / signal / obs / fvgs 完全相等 — 對應 contracts/incremental Invariant I-1 ~ I-4

### Implementation for User Story 4

- [ ] T046 [US4] 修改 `src/smc_features/batch.py`：串接新順序（swings → bos/choch+breaks → ob (吃 breaks) → fvg (with ATR filter)），BatchResult 多回 `breaks` 欄位 — 對應 spec FR-007、FR-014、external task #26
- [ ] T047 [US4] 修改 `src/smc_features/incremental.py`：在 `IncrementalState` 加 dedup state（`used_swing_high_bar_indices`、`used_swing_low_bar_indices`）；`step()` 回傳的 `StepResult` 加 `new_breaks: list[StructureBreak]` — 對應 spec FR-014
- [ ] T048 [US4] 在 `incremental.py` 內部用同一套規則邏輯（建議共用 helper functions 與 batch.py），確保等價性 — 對應 Constitution Gate I-2
- [ ] T049 [US4] 在 `ob.py` 移除 deprecated 的 swing-driven `detect_and_track_obs` wrapper（T029 暫保留的）— 切換完成後清理
- [ ] T050 [US4] 跑 T039 ~ T045，確認全綠
...
- [ ] T051 [P] 在 `tests/integration/test_smc_pipeline.py` 擴充：跨 6 資產（NVDA/AMD/TSM/MU/GLD/TLT）跑 `batch_compute`，驗證每個資產 breaks > 0、obs ≤ breaks、fvgs < 200（NVDA 8 年 daily）— 對應 spec SC-001、SC-002、SC-003
- [ ] T052 [P] 修改 `src/smc_features/README.md`：documenting v2 行為（dedup、break-driven OB、ATR filter）+ 範例 — 對應 Constitution Gate II-1
- [ ] T053 [P] 在 `src/smc_features/structure.py`、`ob.py`、`fvg.py` 每個新函式 docstring 加上規則文字 + 範例（非僅程式碼）— 對應 Constitution Gate II-1
- [ ] T054 跑 `docker compose run --rm dev pytest tests/ --cov=src/smc_features --cov-report=term-missing`，確認 coverage ≥ 90% — 對應 plan.md Technical Context
- [ ] T055 跑 `docker compose run --rm dev mypy src/smc_features/`，確認 0 errors
- [ ] T056 跑 `docker compose run --rm dev ruff check src/smc_features/ tests/`，確認 0 errors
- [ ] T057 執行 quickstart.md Step 2（NVDA real data sanity check），確認數量在 spec 預期範圍：breaks ~50–80、obs ≤ breaks、fvgs < 200 — 對應 spec SC-001 ~ SC-003
- [ ] T058 執行 quickstart.md Step 3（incremental ↔ batch real-data 等價快檢），確認 `OK: incremental == batch` — 對應 SC-006
- [ ] T059 在 `CLAUDE.md` 的 SPECKIT block 標註 008 為 implement 完成、提示下一步「重訓 PPO + retune」屬下個 feature
...
- **Foundational (Phase 2)**：依賴 Phase 1。T003+T004 並行，T005 → T006 → T007 → T008 → T009 → T010 → T011（同檔案 `types.py` 必須序列）。**BLOCKS** 所有 user story phase。
...
- **US4 (Phase 6)**：依賴 US1 + US2 + US3 全部完成（串接整合）。T039 ~ T045 並行 → T046 → T047 → T048 → T049 → T050。
...
Task: T009 擴充 IncrementalState
Task: T010 修改 __init__.py（必須在 T005 之後）
```
...
2. Phase 2 Foundational（T003 ~ T011）—— 必過
