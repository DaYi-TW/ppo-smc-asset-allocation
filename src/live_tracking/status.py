...
class LiveTrackingStatus(BaseModel):
    ...
    def load(cls, path: Path) -> Self:
        # Race-safe read：pipeline 在 atomic-write 前的極短視窗 (<10ms) 內若被
        # 讀到截斷或半舊內容，pydantic 會 raise ValidationError / JSONDecodeError。
        # 給三次短 retry（10ms / 30ms / 100ms）涵蓋 os.replace 完成的時序。
        if not path.exists():
            return cls()
        last_err: Exception | None = None
        for delay in (0.0, 0.01, 0.03, 0.1):
            if delay > 0:
                time.sleep(delay)
            try:
                ...
                last_err = exc
        # 真的壞了 → 重新 raise 最後一次 error
        assert last_err is not None  # for type checker
        raise last_err

    def write(self, path: Path) -> None:
        # Atomic write — `.tmp` + os.replace + fsync。避開 reader 看到截斷 JSON
        # 而 raise ValidationError（曾在 live/refresh 觸發 pipeline 同瞬間，
        # 前端 status polling 收到 500 → Gateway 翻成 503）
